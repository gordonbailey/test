...
def main() -> None:
    ...
    for name, group in bm.groupby("cohort"):
        ...
        cohorts[name] = {
            ...
            "mc": r(pop["campaign_type_breadth"].median(), 1),
        ...
        accounts.append(
            {
                ...
                "ch": r(row.get("campaign_type_breadth")),
                "tcs": r(row.get("top_type_share"), 3),
    ...
    payload = {
        ...
        "narrowTypes": ce.NARROW_FOOTPRINT_TYPES,
        "narrowConcentration": ce.NARROW_FOOTPRINT_CONCENTRATION,
        "nTypes": len(ce.CAMPAIGN_TYPES),
        "campaignTypes": {k: list(v) for k, v in ce.CAMPAIGN_TYPES.items()},
